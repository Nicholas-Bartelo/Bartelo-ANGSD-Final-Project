...
Below we show that there are 540 Ensembl IDs that did not have an external count value and needed to be given their Ensembl ID value instead of the NA value given to these genes. We did that in the code above. 

```{r}
library(VennDiagram)
all_genes_het_wt <- ens_het_wt
overlap_all_genes_het_wt <- intersect(all_genes_het_wt, mapped_names$ensembl_gene_id)
grid.newpage()
venn.plot <- draw.pairwise.venn(area1 = length(mapped_names$ensembl_gene_id),
                           area2 = length(all_genes_het_wt),
                           cross.area = length(overlap_all_genes_het_wt),
                           c("Mapping Ids", "Count Ensembl IDs"),
                           scaled = TRUE,
                           fill = c("green", "blue"),
                           cex = 1.5,
                           cat.cex = 1.5,
...
# Below we find the top 27 upregulated genes that are also significant
...
# Below we find the top 8 downregulated genes that are also significant
...
1 | Aqp12 | Upregulated
2 | Cpb1 | Upregulated
3 | Serpini2 | Upregulated
4 | Amy2a5 | Upregulated
5 | Amy1 | Upregulated
6 | Cela2a | Upregulated
7 | Try10 | Upregulated
8 | Erp27 | Upregulated
9 | Klk1 | Upregulated
10 | Nupr1 | Upregulated
11 | Zg16 | Upregulated
12 | Gm26532 | Upregulated
13 | Ces1d | Upregulated
14 | Ap3s1.ps2 | Upregulated
15 | Ctrl | Upregulated
16 | Rbm15b | Upregulated
17 | Gamt | Upregulated
18 | Gm15344 | Upregulated
19 | Pah | Upregulated
20 | Ddit3 | Upregulated
21 | Slc39a5 | Upregulated
22 | Rnase1 | Upregulated
23 | Cela1 | Upregulated
24 | Clps | Upregulated
25 | Tff2 | Upregulated
26 | Spink1 | Upregulated
27 | Pnliprp1 | Upregulated
28 | Fn1 | Downregulated
29 | Mir6236 | Downregulated
30 | Lars2 | Downregulated
31 | ENSMUSG00000076258 (Gm23935) | Downregulated
32 | ENSMUSG00000079620 (Muc4) | Downregulated
33 | Gm15564 | Downregulated
34 | Gm49839 | Downregulated
35 | ENSMUSG00000106106 (CT010467.1) | Downregulated

Below we create a heatmap using complexheatmap for the top 25 upregulated genes. We see that heterozygous sample 3 has a much greater contrast with the wild-type samples than the other two heterozgyous samples.
...
Below we create a heatmap using complexheatmap for the top 27 upregulated genes being significant in terms of p-value. 
...
Below we create a heatmap using complexheatmap for the top 8 upregulated genes being significant in terms of p-value. 
...
Below we show that there are 559 Ensembl IDs that did not have an external count value and needed to be given their Ensembl ID value instead of the NA value given to these genes. We did that in the code above. 

```{r}
library(VennDiagram)
all_genes_het_db <- ens_het_db
overlap_all_genes_het_db <- intersect(all_genes_het_db, mapped_names$ensembl_gene_id)
grid.newpage()
venn.plot <- draw.pairwise.venn(area1 = length(mapped_names$ensembl_gene_id),
                           area2 = length(all_genes_het_db),
                           cross.area = length(overlap_all_genes_het_db),
                           c("Mapping Ids", "Count Ensembl IDs"),
                           scaled = TRUE,
                           fill = c("green", "blue"),
                           cex = 1.5,
...
# Below we find the top 50 upregulated genes that are also significant
...
# Below we find the top 50 downregulated genes that are also significant
...
Below we create a heatmap using complexheatmap for the top 50 upregulated genes being significant in terms of p-value. 
...
Below we create a heatmap using complexheatmap for the top 50 downregulated genes being significant in terms of p-value. 
...
Below we show that there are 541 Ensembl IDs that did not have an external count value and needed to be given their Ensembl ID value instead of the NA value given to these genes. We did that in the code above. 

```{r}
library(VennDiagram)
all_genes_db_wt <- ens_db_wt
overlap_all_genes <- intersect(all_genes_db_wt, mapped_names$ensembl_gene_id)
grid.newpage()
venn.plot <- draw.pairwise.venn(area1 = length(mapped_names$ensembl_gene_id),
                           area2 = length(all_genes_db_wt),
                           cross.area = length(overlap_all_genes),
                           c("Mapping Ids", "Count Ensembl IDs"),
                           scaled = TRUE,
                           fill = c("green", "blue"),
                           cex = 1.5,
...
Below we create a heatmap using complexheatmap for the top 50 upregulated genes without necessarily being significant in terms of p-value. We see that diabetes sample 2 is more like the wild type samples than the other two diabetes samples. This makes sense because it is what we saw from the PCA as well, as diabetes sample 2 was in between the wild type and diabetes samples, but closer to the other diabetes samples.
...
# Comparing Results To See If This Is Good Model For Dedifferentiation
